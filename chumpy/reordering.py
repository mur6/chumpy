        ...
        if wrt is self.a:
            if False:
                ...
                return LinearOperator(
                    (self.size, wrt.size),
                    lambda x: self.reorder(x.reshape(self.a.shape)).ravel(),
                )
            else:
                a = self.a
                asz = a.size
                ...
                if key not in self.dr_lookup or key is None:
                    ...
                    shape = JS.shape
                    self.dr_lookup[key] = sp.csc_matrix(
                        (data, (IS, JS.ravel())), shape=(self.r.size, wrt.r.size)
                    )
...
class Sort(Reorder):
    ...
    def reorder(self, a):
        return np.sort(a, self.axis, self.kind, self.order)

    def unique_reorder_id(self):
        return None
...
class Tile(Reorder):
    ...
    def reorder(self, a):
        return np.tile(a, self.reps)

    def unique_reorder_id(self):
        return (self.a.shape, tuple(self.reps))
...
class Diag(Reorder):
    ...
    def reorder(self, a):
        return np.diag(a, self.k)

    def unique_reorder_id(self):
        return (self.a.shape, self.k)
...
class Repeat(Reorder):
    ...
    def reorder(self, a):
        return np.repeat(a, self.repeats, self.axis)

    def unique_reorder_id(self):
        return (self.repeats, self.axis)
...
class transpose(Reorder):
    ...
    def unique_reorder_id(self):
        return (self.a.shape, None if self.axes is None else tuple(self.axes))
...
class rollaxis(Reorder):
    ...
    def reorder(self, a):
        return np.rollaxis(a, axis=self.axis, start=self.start)

    def unique_reorder_id(self):
        return (self.a.shape, self.axis, self.start)
...
class swapaxes(Reorder):
    ...
    def reorder(self, a):
        return np.swapaxes(a, axis1=self.axis1, axis2=self.axis2)

    def unique_reorder_id(self):
        return (self.a.shape, self.axis1, self.axis2)
...
class Roll(Reorder):
    ...
    def reorder(self, a):
        return np.roll(a, self.shift, self.axis)

    def unique_reorder_id(self):
        return (self.shift, self.axis)
    ...
    def compute_dr_wrt(self, wrt):
        ...
        for term in self.our_terms:
            ...
            if term is wrt:
                ...
                IS += [
                    np.swapaxes(
                        self.everything[offset : offset + tsz], self.axis, 0
                    ).ravel()
                ]
            offset += tsz
