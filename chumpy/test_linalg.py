...
class TestLinalg(unittest.TestCase):
    ...
    def test_lstsq(self):
        ...
        for shape in shapes:
            for b2d in True, False:
                A = (np.random.rand(np.prod(shape)) - 0.5).reshape(shape)
                if b2d:
    ...
    def test_pinv(self):
        ...
        data = (np.random.rand(12) - 0.5).reshape((3, 4))
        pc_tall = Pinv(data)
        pc_wide = Pinv(data.T)
    ...
    def test_svd(self):
        ...
        # print ratio
        self.assertTrue(np.max(np.abs(ratio - 1.0)) < 1e-4)
        ...
        ratio = diff_emp / diff_pred
        # print ratio
        self.assertTrue(np.max(np.abs(ratio - 1.0)) < 1e-2)
    ...
    def test_inv3(self):
        ...
        self.assertTrue(
            np.max(np.abs(output_diff_emp.ravel() - output_diff_pred.ravel()))
            < eps * 1e-3
        )
        self.assertTrue(np.max(np.abs(mtx1_inv.r - np.linalg.inv(mtx1.r)).ravel()) == 0)
    ...
    def chkd(self, obj, parm, eps=1e-14):
        ...
        if True:
            diff = (np.random.rand(parm.size) - 0.5).reshape(parm.shape)
        else:
        ...
        diff_emp = (r_upper - r_lower) / (eps * 2.0)
