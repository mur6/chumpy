...
class Madsen(Ch):
    ...
    def compute_r(self):
        x1 = self.x.r[0]
        x2 = self.x.r[1]
        result = np.array((x1**2 + x2**2 + x1 * x2, np.sin(x1), np.cos(x2)))
        return result
...
class TestOptimization(unittest.TestCase):
    def test_dogleg_rosen(self):
        ...
        minimize(
            ...
            x0=freevars,
            method="dogleg",
            options={"maxiter": 337, "disp": False},
        )
        self.assertTrue(freevars[0].r[0] == 1.0)
        self.assertTrue(freevars[1].r[0] == 1.0)

    def test_dogleg_madsen(self):
        obj = Madsen(x=Ch(np.array((3.0, 1.0))))
        minimize(
            fun=obj, x0=[obj.x], method="dogleg", options={"maxiter": 34, "disp": False}
        )
    ...
    def test_bfgs_madsen(self):
        ...
        self.assertLess(obj.r / 2.0, 0.4)

        # Optimize with chumpy's minimize (which uses scipy's bfgs).
        obj.x = x0
        minimize(
            fun=obj, x0=[obj.x], method="bfgs", options={"maxiter": 8, "disp": False}
        )
        self.assertLess(obj.r / 2.0, 0.4)
    ...
    def test_nested_select(self):
        ...
        minimize(
            beales(x1, y1), x0=[x1[1:4], y1], method="dogleg", options={"disp": False}
        )
