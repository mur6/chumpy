...
for rtn in ["argmax", "nanargmax", "argmin", "nanargmin"]:
    exec(
        'def %s(a, axis=None) : return np.%s(a.r, axis) if hasattr(a, "compute_r") else np.%s(a, axis)'
        % (rtn, rtn, rtn)
    )
    __all__ += [rtn]
...
for rtn in ["argwhere", "nonzero", "flatnonzero"]:
    exec(
        'def %s(a) : return np.%s(a.r) if hasattr(a, "compute_r") else np.%s(a)'
        % (rtn, rtn, rtn)
    )
    __all__ += [rtn]
...
class Cross(Ch):
    ...
    def compute_r(self):
        return np.cross(
            self.a.r, self.b.r, self.axisa, self.axisb, self.axisc, self.axis
        )
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.a:
            ...
            result = np.cross(
                tiled_identity, rp, self.axisa, self.axisb, self.axisc, self.axis
            )
...
class UnaryElemwise(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        if wrt is self.x:
            ...
            return (
                sp.diags([result.ravel()], [0])
                if len(result) > 1
                else np.atleast_2d(result)
            )
...
class sqrt(UnaryElemwise):
    _r = np.sqrt
    _d = lambda self, x: 0.5 * my_power(x, -0.5)
...
class arcsin(UnaryElemwise):
    _r = np.arcsin
    _d = lambda self, x: np.reciprocal(np.sqrt(1.0 - np.square(x)))
...
class tan(UnaryElemwise):
    _r = np.tan
    _d = lambda self, x: np.reciprocal(np.cos(x) ** 2.0)


class arctan(UnaryElemwise):
    _r = np.arctan
    _d = lambda self, x: np.reciprocal(np.square(x) + 1.0)
...
class clip(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        if wrt is self.a:
            result = np.asarray(
                (self.r != self.a_min) & (self.r != self.a_max), np.float64
            )
            return (
                sp.diags([result.ravel()], [0])
                if len(result) > 1
...
def var(a, axis=None, dtype=None, out=None, ddof=0, keepdims=False):
    if dtype != None or out != None or ddof != 0 or keepdims != False:
        raise NotImplementedException(
            "Unimplemented for non-default dtype, out, ddof, and keepdims."
        )
    return mean(a**2.0, axis=axis)
...
class SumOfSquares(Ch):
    ...
    def compute_r(self):
        return np.sum(self.x.r.ravel() ** 2.0)

    def compute_dr_wrt(self, wrt):
        if wrt is self.x:
            return row(self.x.r.ravel() * 2.0)
...
def _broadcast_matrix(a, b, wrt, data):
    ...
    if len(set((a.shape, b.shape))) == 1:
        ...
        if uid not in _bs_setup_data1:
            asz = a.size
            IS = np.arange(asz)
            _bs_setup_data1[uid] = sp.csc_matrix(
                (np.empty(asz), (IS, IS)), shape=(asz, asz)
            )
        ...
        if uid not in _bs_setup_data2:
            ...
            b2 = (
                ...
                if (wrt is b and wrt is not a)
                else np.zeros(b.shape)
            )
...
def broadcast_shape(a_shape, b_shape):
    ...
    raise Exception(
        "This function is probably a bad idea, because shape is not cached and overquerying can occur."
    )
...
class add(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        m = 2.0 if self.a is self.b else 1.0
...
class subtract(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        m = 1.0 if wrt is self.a else -1.0
        return _broadcast_matrix(self.a, self.b, wrt, m)
...
class power(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        result = []
        if wrt is self.x:
            result.append(pow * self.safe_power(x, pow - 1.0))
        if wrt is self.pow:
...
class A_extremum(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        if wrt is self.a:
            ...
            JS = np.asarray(
                np.round(mn + stride * self.argf(self.a.r, axis=self.axis)),
                dtype=np.uint32,
            ).ravel()
...
class multiply(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        b2 = (
            self.b.r if (wrt is self.a and wrt is not self.b) else np.ones(self.b.shape)
        )
        data = (a2 * b2).ravel()
...
class dot(Ch):
    ...
    def compute_d1(self):
        ...
        ar = (
            row(self.a.r)
            if len(self.a.r.shape) < 2
            else self.a.r.reshape((-1, self.a.r.shape[-1]))
        )
        br = (
            col(self.b.r)
            if len(self.b.r.shape) < 2
            else self.b.r.reshape((self.b.r.shape[0], -1))
        )
...
source_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "np_tensordot.py"
)
