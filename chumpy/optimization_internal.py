...
def vstack(x):
    x = [
        a
        if not isinstance(a, sp.linalg.interface.LinearOperator)
        else a.dot(np.eye(a.shape[1]))
        for a in x
    ]
    return (
        sp.vstack(x, format="csc") if any([sp.issparse(a) for a in x]) else np.vstack(x)
    )
...
def hstack(x):
    ...
    return (
        sp.hstack(x, format="csc") if any([sp.issparse(a) for a in x]) else np.hstack(x)
    )
        ...
        if "x" in which:
            ...
            for idx, freevar in enumerate(self.free_variables):
                ...
                if isinstance(self.free_variables[idx], ch.Select):
                    ...
                    self.free_variables[idx].__setattr__(
                        "x",
                        self.x.r[rng].copy().reshape(self.free_variables[idx].x.shape),
                        _giter,
                    )
                else:  # a number
...
def setup_sparse_solver(sparse_solver):
    ...
    if callable(sparse_solver):
        ...
        raise Exception(
            "sparse_solver argument must be either a string in the set (%s) or have the api of scipy.sparse.linalg.spsolve."
            % ", ".join(list(_solver_fns.keys()))
        )
...
def setup_objective(obj, free_variables, on_step=None, disp=True, make_dense=False):
    ...
    obj = ChInputsStacked(
        obj=obj, free_variables=free_variables, x=x, make_dense=make_dense
    )
...
class DoglegState(object):
    ...
    def start_iteration(self):
        ...
        self.d_sd = (
            np.linalg.norm(self.g) ** 2
            / np.linalg.norm(self.J.dot(self.g)) ** 2
            * self.g
        ).ravel()
        self.d_gn = None
    ...
    def update_step(self):
        ...
        if self.delta is not None and np.linalg.norm(self.d_sd) >= self.delta:
            ...
            self.d_dl = np.array(
                self.delta / np.linalg.norm(self.d_sd) * self.d_sd
            ).ravel()
        else:
    ...
    def beta_multiplier(self):
        ...
        pnow = (
            diff.T.dot(diff) * delta_sq
            + self.d_gn.T.dot(self.d_sd) ** 2
            - np.linalg.norm(self.d_gn) ** 2 * sqnorm_sd
        )
        return float(delta_sq - sqnorm_sd) / float(
            (diff).T.dot(self.d_sd) + np.sqrt(pnow)
        )
    ...
    def updateGN(self):
        ...
        if sp.issparse(self.A):
            ...
            pif(
                "sparse solve...sparsity infill is %.3f%% (hessian %dx%d)"
                % (
                    100.0 * self.A.nnz / (self.A.shape[0] * self.A.shape[1]),
                    self.A.shape[0],
                    self.A.shape[1],
                )
            )
    ...
    class Trial(object):
        ...
        def __init__(self, proposed_r, state):
            ...
            if self.rho > 0:
                with warnings.catch_warnings():
                    ...
                    predicted_improvement = 2.0 * state.g.T.dot(
                        state.d_dl
                    ) - state.d_dl.T.dot(state.A.dot(state.d_dl))
        ...
        def improvement(self):
            return (
                np.linalg.norm(self.state.r) ** 2 - np.linalg.norm(self.r) ** 2
            ) / np.linalg.norm(self.state.r) ** 2

    def trial_r(self, proposed_r):
        return self.Trial(proposed_r, self)

    def updateRadius(self, rho, lb=0.05, ub=0.9):
        if rho > ub:
...
def minimize_dogleg(
    ...
    obj, callback = setup_objective(
        obj, free_variables, on_step=on_step, disp=disp, make_dense=treat_as_dense
    )
    ...
    while not state.done:
        ...
        while True:
            ...
            if state.step_size <= e_2 * np.linalg.norm(state.p):
                stop(
                    "stopping because of small step size (norm_dl < %.2e)"
                ...
                if trial.is_improvement:
                    ...
                    callback()
                    if e_3 > 0.0 and trial.improvement < e_3:
