...
def make_row(a, b, c, b_color, c_color):
    global src
    src += (
        '<tr><td>%s</td><td style="background-color:%s">%s</td><td style="background-color:%s">%s</td></tr>'
...
def hd2(s):
    ...
    src += (
        '<tr><td colspan=3 style="background-color:black;color:white"><h2 style="margin-bottom:0;">%s</h2></td></tr>'
        % (s,)
    )
...
def main():
    ...
    r(
        "array",
        {"object": [1, 2, 3]},
        {"dtype": np.float64, "order": "C", "subok": False, "ndmin": 2},
    )
    ...
    r(
        "fromfunction",
        {"function": lambda i, j: i + j, "shape": (3, 3)},
        {"dtype": np.float64},
    )
    ...
    r("cos", (np.arange(3),), {})
    r("tan", (np.arange(3),), {})
    r("arcsin", (np.arange(3) / 3.0,), {})
    r("arccos", (np.arange(3) / 3.0,), {})
    r("arctan", (np.arange(3) / 3.0,), {})
    ...
    r("cosh", (np.arange(3),), {})
    r("tanh", (np.arange(3),), {})
    r("arcsinh", (np.arange(3) / 9.0,), {})
    r("arccosh", (-np.arange(3) / 9.0,), {})
    r("arctanh", (np.arange(3) / 9.0,), {})
    ...
    r("beta", (0.5, 0.5), {}, **extra_args)
    r("binomial", (0.5, 0.5), {}, **extra_args)
    r("chisquare", (0.5,), {}, **extra_args)
    r(
        "dirichlet",
        (
            (10, 5, 3),
    ...
    r(
        "digitize",
        (np.array([0.2, 6.4, 3.0, 1.6]), np.array([0.0, 1.0, 2.5, 4.0, 10.0])),
        {},
    )
