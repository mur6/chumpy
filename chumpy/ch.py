...
def _props_for(cls):
    if cls not in _props_for_dict:
        _props_for_dict[cls] = set(
            [p[0] for p in inspect.getmembers(cls, lambda x: isinstance(x, property))]
        )
...
def _dep_props_for(cls):
    if cls not in _dep_props_for_dict:
        _dep_props_for_dict[cls] = [
            p
            for p in inspect.getmembers(cls, lambda x: isinstance(x, property))
            if hasattr(p[1].fget, "deps")
        ]
...
def _check_kw_conflict(cls):
    if cls not in _kw_conflict_dict:
        _kw_conflict_dict[cls] = Ch._reserved_kw.intersection(
            set(cls.terms).union(set(cls.dterms))
        )
    if _kw_conflict_dict[cls]:
        raise Exception(
            "In class %s, don't use reserved keywords in terms/dterms: %s"
            % (
                str(cls),
                str(kw_conflict),
            )
...
class Ch(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        object.__setattr__(
            result, "_cache", {"r": None, "drs": weakref.WeakKeyDictionary()}
        )
        ...
        defs = {
            p.name: deepcopy(p.default)
            for p in cls.parm_declarations()
    ...
    def sid(self):
        ...
        return (
            self.__class__,
            tuple([(k, id(self.__dict__[k])) for k in pnames if k in self.__dict__]),
        )
    ...
    def print_labeled_residuals(
        self, print_newline=True, num_decimals=2, where_to_print=None
    ):
        if where_to_print is None:
            where_to_print = sys.stderr
        if hasattr(self, "label"):
            where_to_print.write(
                ("%s: %." + str(num_decimals) + "e | ")
                % (self.label, np.sum(self.r**2))
            )
        for dterm in self.dterms:
            dt = getattr(self, dterm)
            if hasattr(dt, "dterms"):
                dt.print_labeled_residuals(
                    print_newline=False, where_to_print=where_to_print
                )
        if print_newline:
            where_to_print.write(
                ("%." + str(num_decimals) + "e\n") % (np.sum(self.r**2),)
    ...
    def size(self):
        ...
        return np.prod(
            self.shape
        )  # may be cheaper since it doesn't always mean grabbing "r"
    ...
    def __setattr__(self, name, value, itr=None):
        ...
        name_in_props = name in _props_for(
            self.__class__
        )  # [p[0] for p in inspect.getmembers(self.__class__, lambda x : isinstance(x, property))]
            ...
            if id(next) not in done:
                ...
                for parent, parent_dict in list(next._parents.items()):
                    object.__setattr__(
                        parent,
                        "_dirty_vars",
                        parent._dirty_vars.union(parent_dict["varnames"]),
                    )
    ...
    def clear_cache_wrt(self, wrt, itr=None):
        ...
        if itr is None or itr != self._itr:
            for parent, parent_dict in list(self._parents.items()):
                if wrt in parent._cache["drs"] or (
                    hasattr(parent, "dr_cached") and wrt in parent.dr_cached
                ):
    ...
    def replace(self, old, new):
        if hasattr(old, "dterms") != hasattr(new, "dterms"):
            raise Exception(
                'Either "old" and "new" must both be "Ch", or they must both be neither.'
            )

        for term_name in [
            t for t in list(self.dterms) + list(self.terms) if hasattr(self, t)
        ]:
    ...
    def is_dr_wrt(self, wrt):
        if type(self) == Ch:
            return wrt is self
        dterms_we_have = [
            getattr(self, dterm) for dterm in self.dterms if hasattr(self, dterm)
        ]
    ...
    def __setitem__(self, key, value, itr=None):
        ...
        # minimize_dogleg(obj=self.__getitem__(key) - value, free_variables=kids, show_residuals=False)
            ...
            if sp.issparse(t1):
                t1 = np.array(t1.todense())
            if sp.issparse(t2):
                t2 = np.array(t2.todense())
    ...
    def r(self):
        ...
        if self._cache["r"] is None:
            self._cache["r"] = np.asarray(
                np.atleast_1d(self.compute_r()), dtype=np.float64, order="C"
            )
        ...
        try:
            ...
            if isinstance(rhs, LinearOperator):
                return LinearOperator(
                    (lhs.shape[0], rhs.shape[1]), lambda x: lhs.dot(rhs.dot(x))
                )
            ...
            if tm_maybe_sparse() > 0.1:
                pif(
                    "convert_inputs_to_sparse_if_necessary in {}sec".format(
                        tm_maybe_sparse()
                    )
                )
    ...
    def lmult_wrt(self, lhs, wrt):
        ...
        for k in set(self.dterms):
            ...
            if hasattr(p, "dterms") and p is not wrt and p.is_dr_wrt(wrt):
                ...
                indirect_dr = p.lmult_wrt(
                    self._superdot(lhs, self._compute_dr_wrt_sliced(p)), wrt
                )
    ...
    def compute_lop(self, wrt, lhs):
        ...
        if dr is None:
            return None
        return (
            self._superdot(lhs, dr)
            if not isinstance(lhs, LinearOperator)
            else lhs.matmat(dr)
        )
    ...
    def dr_wrt(self, wrt, reverse_mode=False, profiler=None):
        ...
        if result is not None and not sp.issparse(result):
            ...
            if (
                nonzero == 0
                or hasattr(result, "size")
                and result.size / float(nonzero) >= 10.0
            ):
                ...
                gc.collect()
                pif(
                    "converting result to sparse in {}sec".format(
        ...
        if (
            (result is not None)
            and (not sp.issparse(result))
            and (not isinstance(result, LinearOperator))
        ):
        ...
        if tm_dr_wrt() > 0.1:
            pif(
                "dx of {} wrt {} in {}sec, sparse: {}".format(
                    self.short_name, wrt.short_name, tm_dr_wrt(), sp.issparse(result)
                )
            )
            ...
            for dterm in self.dterms:
                if hasattr(self, dterm):
                    ...
                    if hasattr(dtval, "dterms") or hasattr(dtval, "terms"):
                        child_label = (
                            getattr(dtval, "label")
                            if hasattr(dtval, "label")
                            else dterm
                        )
                        child_label = "%s (%s)" % (
                            child_label,
                            str(dtval.__class__.__name__),
                        )
                        src = "aaa%d" % (id(self))
                        dst = "aaa%d" % (id(dtval))
                        ...
                        s = ""
                        color = (
                            color_mapping[dtval._status]
                            if hasattr(dtval, "_status")
                            else "grey"
                        )
    ...
    def show_tree_wrt(self, wrt):
            ...
            for dterm in self.dterms:
                if hasattr(self, dterm):
                    ...
                    if hasattr(dtval, "dterms") or hasattr(dtval, "terms"):
                        ...
                        result += ['%s [label="%s"];' % (src, my_name)]
                        if (
                            wrt in dtval._cache["drs"]
                            and dtval._cache["drs"][wrt] is not None
                        ):
                            ...
                            result += [
                                '%s [label="%s\n is_sparse: %s\nsize: %d\nnonzero: %d"];'
                                % (dst, child_label, issparse, size, nonzero)
                            ]
                        else:
        ...
        dot_file_contents = "digraph G {\n%s\n}" % "\n".join(
            list(set(string_for(self, "root", wrt)))
        )
            ...
            if cachelim != np.inf and hasattr(self, "_cache") and "drs" in self._cache:
                ...
                for dtval, jac in list(self._cache["drs"].items()):
                    ...
                    if sz > (cachelim * 1024 * 1024):
                        result += [
                            '%s -> %s [style=dotted,color="<<<%d>>>"];' % (src, dst, sz)
                        ]
                    #
        ...
        if cachelim != np.inf:
            ...
            if len(strs) > 0:
                ...
                for s in strs:
                    ...
                    dot_file_contents = re.sub(
                        "<<<%s>>>" % s,
                        '#%02x%02x%02x",label="%d%s'
                        % (szpct * 255, 0, (1 - szpct) * 255, sz, unit),
                        dot_file_contents,
                    )
    ...
    def __radd__(self, other):
        return add(a=other, b=self)

    def __sub__(self, other):
        return subtract(a=self, b=other)

    def __rsub__(self, other):
        return subtract(a=other, b=self)

    def __mul__(self, other):
        return multiply(a=self, b=other)

    def __rmul__(self, other):
        return multiply(a=other, b=self)

    def __div__(self, other):
        return divide(x1=self, x2=other)

    def __truediv__(self, other):
        return divide(x1=self, x2=other)

    def __rdiv__(self, other):
        return divide(x1=other, x2=self)

    def __pow__(self, other):
        return power(x=self, pow=other)

    def __rpow__(self, other):
        return power(x=other, pow=self)
