...
class Inv(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if Ainv.ndim <= 2:
            ...
            result = np.dstack(
                [-np.kron(Ainv[i], AinvT[i]).T for i in range(Ainv.shape[0])]
            ).T
...
class SvdD(Ch):
    ...
    def UDV(self):
        ...
        result[1][np.abs(result[1]) < np.spacing(1)] = 0.0
        return result
...
class SvdU(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        if wrt is self.x:
            ...
            #     ).dr_wrt(self.svd_d.x)
            return (
                NanDivide(self.svd_d.x.dot(self.svd_v), self.svd_d.reshape((1, -1)))
            ).dr_wrt(self.svd_d.x)
