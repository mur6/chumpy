...
class TestCh(unittest.TestCase):
    ...
    def test_nandivide(self):
        ...
        bar[2, 2] = 0
        self.assertEqual(ch.NanDivide(foo, bar)[2, 2].r, 0.0)
        foo[2, 2] = 0
        self.assertEqual(ch.NanDivide(foo, bar)[2, 2].r, 0.0)
    ...
    def test_tensordot(self):
        an = np.arange(60.0).reshape(3, 4, 5)
        bn = np.arange(24.0).reshape(4, 3, 2)
        cn = np.tensordot(an, bn, axes=([1, 0], [0, 1]))

        ac = ch.arange(60.0).reshape(3, 4, 5)
        bc = ch.arange(24.0).reshape(4, 3, 2)
    ...
    def test_cross(self):
        ...
        eps = 1.0
        step = (np.random.rand(30) - 0.5).reshape((10, 3)) * eps
    ...
    def test_sum_mean_std_var(self):
        for fn in [ch.sum, ch.mean, ch.var, ch.std]:
            ...
            data2 = ch.array(
                data1.r + 0.1 * np.random.rand(data1.size).reshape(data1.shape)
            )
            ...
            if fn in [ch.std, ch.var]:
                ...
                self.assertTrue(
                    result1.dr_wrt(data1) is dr0
                )  # changing values shouldn't force recompute
                result1.axis = 1
                self.assertTrue(result1.dr_wrt(data1) is not dr0)

        self.assertEqual(
            ch.mean(ch.eye(3), axis=1).ndim, np.mean(np.eye(3), axis=1).ndim
        )
        self.assertEqual(
            ch.mean(ch.eye(3), axis=0).ndim, np.mean(np.eye(3), axis=0).ndim
        )
    ...
    def test_cumsum(self):
        a = ch.array([1.0, 5.0, 3.0, 7.0])
        cs = ch.cumsum(a)
        r1 = cs.r
        dr = cs.dr_wrt(a)
        diff = (ch.random.rand(4) - 0.5) * 0.1
        a.x += diff.r
        pred = dr.dot(diff.r)
        gt = cs.r - r1
    ...
    @unittest.skip(
        "We are using LinearOperator for this for now. Might change back though."
    )
    ...
    def test_serialization(self):
        ...
        self.assertTrue(
            list(tmp.a._parents.keys())[0] == list(tmp.b._parents.keys())[0]
        )
    ...
    def test_amax(self):
        ...
        arr.flat[:] = np.sin(np.arange(arr.size) * 1000.0)
    ...
    def test_maximum(self):
        ...
        self.assertTrue(the_max.r.ravel()[0] == 1.0)
        self.assertTrue(the_max.dr_wrt(the_max.a)[0, 0] == 1.0)
        self.assertTrue(the_max.dr_wrt(the_max.b)[0, 0] == 1.0)
    ...
    def test_indexing(self):
        ...
        little = big[1:3, 3:6]
        self.assertTrue(
            np.max(np.abs(little.r - np.array([[9, 10, 11], [15, 16, 17]]))) == 0
        )

        little = big[5]
        self.assertTrue(np.max(np.abs(little.r - np.arange(30, 36))) == 0)
        self.assertTrue(
            np.max(np.abs(sp.coo_matrix(little.dr_wrt(big)).col - np.arange(30, 36)))
            == 0
        )
        ...
        little = big[2, 3:5]
        self.assertTrue(np.max(np.abs(little.r - np.array([15, 16]))) == 0.0)
        _ = little.dr_wrt(big)
        ...
        for MT in [False, True]:
            ...
            for k in range(len(vals)):
                ...
                without_nans = np.nonzero(
                    np.logical_not(np.isnan(theano_derivative.flatten()))
                )[0]
